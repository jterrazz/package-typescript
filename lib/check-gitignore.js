 ...
 * `.git` directory in a clone, a `.git` file in a worktree). A workspace whose `lint` delegates to members runs this
...
/**
 * A repository boundary: the `.git` a clone carries as a DIRECTORY and a
 * worktree as a FILE. Both spellings mean the same thing to this gate — the
 * tree above belongs to somebody else.
 */
 ...
 *
 * The walk stops at the nearest repository boundary and never passes it. A
 * clone checked out INSIDE another repository — a workbench under
 * `home/<brand>/work/`, a vendored tree — is not a member of the tree it
 * happens to sit in, and the `.gitignore` above that boundary answers for a
 * project whose artefacts are none of this one's business. `root` carrying its
 * own `.git` therefore has no ancestor at all: it IS the repository.
 */
