...
/** A `_` opens a row: a fixture, a golden, a template — another project's tree. */
const OF_THE_ROW = (path) => path.split('/').some((segment) => segment.startsWith('_'));

const LINK = /!?\[[^\]]*]\((?<target>[^)\s]+)\)/g;
 ...
 */
const BACKTICK_PATH = /`(?<path>(?:apps|bin|lib|packages|presets|specs|tests)\/[a-zA-Z\d._/-]+)`/g;

/** A fenced example illustrates; it does not cite. */
const withoutFences = (markdown) => markdown.replaceAll(/^```.*?^```/gms, '');
...
function auditPage(root, path, markdown) {
    ...
    for (const match of cited.matchAll(BACKTICK_PATH)) {
        const target = match.groups.path.replace(/\/$/, '');
