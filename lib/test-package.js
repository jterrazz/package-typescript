 ...
 * `--at-least`, when it is that version or newer — 1 otherwise. `--floor`
 * prints the release the checker's own flags arrive in, so the two callers
 * that gate on it read one number rather than each keeping a copy.
 */
