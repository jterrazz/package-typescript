...
const manifest = require(resolve(PACKAGE_ROOT, 'package.json'));
...
const rows = [
    ['node', process.version.replace(/^v/, ''), manifest.engines?.node ?? '*'],
    ...
    ['oxlint-tsgolint', installedVersion('oxlint-tsgolint'), declared['oxlint-tsgolint'] ?? '*'],
...
for (const [name, installed, range] of rows) {
    const verdict = verdictOf(installed, range);
    failed ||= verdict === 'old' || verdict === 'absent';
    warned ||= verdict === 'newer';
...
if (failed) {
    ...
    stdout.write('A tool ahead of its range may be fine; the toolchain does not get to decide.\n');
} else {
