...
export const SOURCE = /\.(?:astro|[cm]?[jt]sx?|svelte|vue)$/;
...
const BIOME = new RegExp(`${COMMENT}biome-ignore\\b`);
...
function rulesOf(tail) {
    ...
    return named
        ...
        .split(',')
        .map((name) => name.trim())
        .filter((name) => /^[\w-]+(?:\/[\w-]+)*$/.test(name) && name !== '');
}

/** Whether the directive carries a reason after the separator oxlint reads. */
const hasReason = (tail) => (tail.split(REASON)[1] ?? '').replace(/\*\/\s*$/, '').trim() !== '';
...
function auditLine(line, number, config) {
    ...
    for (const match of line.matchAll(DISABLE)) {
        ...
        if (tool === 'es') {
            ...
            const settled =
                named.length > 0 && named.every((name) => standingOf(name, config) !== 'dead');
            if (settled) {
...
if (import.meta.main) {
    ...
    for (const path of sources) {
        const text = readText(root, path);
        if (text === null || !/(?:es|ox)lint-disable|@ts-|biome-ignore/.test(text)) {
            continue;
        }
        ...
        for (const [index, line] of lines.entries()) {
            ...
            for (const violation of found) {
                if (isFix && fixed !== line && violation.rule === 'suppressions-spelling') {
                    continue;
                }
