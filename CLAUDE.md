...
- `specs/cli/check/` — twenty-two documents, each stating the whole combined `check`/`fix` output (D11). Where a fixture makes a tool refuse for a reason the scenario is not about — a knip case with no tsconfig, so tsc prints its manual — the document spans that block with `{{any}}` and a comment naming whose it is.
